  ...
  return (
    <div className="bg-white p-7 rounded-[2.5rem] mindskills-shadow border border-[#F1F3FF] flex-1 group hover:-translate-y-1 transition-all duration-300">
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="text-[10px] font-black text-slate-300 uppercase tracking-widest bg-slate-50 px-3 py-1 rounded-full">LIVE DATA</div>
      </div>
      {isLoading ? (
      ...
      <div className="bg-white rounded-[3.5rem] p-10 md:p-14 border border-[#F1F3FF] mindskills-shadow flex flex-col md:flex-row items-center gap-12 relative overflow-hidden">
        ...
        <div className="w-72 h-72 bg-slate-50 rounded-[3.5rem] flex items-center justify-center relative overflow-hidden group shadow-inner">
           ...
           <div className="text-center relative z-10">
              ...
              <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] mt-1">Platform Efficiency</p>
           </div>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        <StatCard title="Total Revenue" value={stats?.totalBilled} icon={<Trophy />} color="blue" isLoading={loading} />
        <StatCard title="Total Customers" value={stats?.customers} icon={<Users />} color="teal" isLoading={loading} />
        <StatCard title="Ongoing Projects" value={stats?.projects} icon={<Activity />} color="purple" isLoading={loading} />
        <StatCard title="Feedback Rating" value={stats?.rating} icon={<Star />} color="orange" isLoading={loading} />
      </div>
